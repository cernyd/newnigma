...
if __name__ == '__main__':
    ...
    argument_data = (
        ('--test', dict(
            help="runs tests before launching the simulator to"
                 "ensure it works correctly", dest="run_tests")),
        ('--cli', dict(help="launches the simulator in the command line mode",
                       dest='cli')),
    ...
    cli_data = (
        ('--model', dict(help="available Enigma models: Enigma1, EnigmaM3,"
                              "EnigmaM4,  Norenigma, EnigmaG, EnigmaD, "
                              "EnigmaK, SwissK, Railway, Tirpitz",
                         nargs=1, dest='model')),
        ('--rotors', dict(help="rotors that will be used", nargs='+',
                          metavar='rotor')),
        ('--positions', dict(
            help="starting rotor positions", nargs='+',
            default=None, metavar='position')),
        ('--ring_settings', dict(
            help="rotor ring settings",
            nargs='+', default=None, metavar='ring_setting')),
        ('--reflector', dict(help="reflector that will be used", nargs=1)),
        ('--reflector_position', dict(
            help="reflector position (only available in EnigmaD, EnigmaK,"
                 "SwissK, EnigmaG, Railway, Tirpitz)",
                 nargs=1, default=None, metavar='position')),
        ('--reflector_pairs', dict(
            help="reflector wiring pairs for UKW-D (pairs do not "
                 "correspond with real wiring!)",
                 nargs='*', default=None, metavar='pair')),
        ('--plug_pairs', dict(
        ...
        ('--uhr', dict(
            ...
            nargs=1, default=None, metavar='position')),
        ('--message', dict(
            help="message to be encrypted", nargs=1, dest='message'))
    )
